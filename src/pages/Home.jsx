...
import { getMostBorrowedBooks } from "../services/librarianApi";
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        const res = await getMostBorrowedBooks(); // imported from `../services/librarianApi`
        const fetchedBooks = res.data.data.map((item) => ({
          img: item.bookDetails.coverImagePath || "/fallback.jpg",
          title: item.bookDetails.title,
          text: `by ${item.bookDetails.author || "Unknown Author"}`,
        }));
        setCards(fetchedBooks);
      } catch (err) {
  ...
  const MAX_DOTS = 3;
  const limitedGroups = Math.min(totalGroups, MAX_DOTS);
  useEffect(() => {
    setCurrentIndex((prev) => Math.min(prev, limitedGroups - 1));
  }, [limitedGroups]);



  const groupedCards = Array.from({ length: limitedGroups }, (_, i) =>
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) => (prev + 1) % limitedGroups);
  };

  const prevSlide = () => {
    setCurrentIndex((prev) => (prev - 1 + limitedGroups) % limitedGroups);
  };

  useEffect(() => {
    if (isHovered || limitedGroups === 0) return;
  ...
  }, [isHovered, limitedGroups]);
  useEffect(() => {
    if (limitedGroups > 0) {
      const timeout = setTimeout(() => {
        nextSlide();
      }, 1000); // move to slide 1 after 1 second
          ...
          <section className="py-16 text-black">
            <div className="max-w-7xl mx-auto px-6">
              <div className="relative">
                              ...
                              <div className="bg-white backdrop-blur-sm border border-white/10 rounded-2xl shadow-2xl overflow-hidden h-full group-hover:bg-white/10 group-hover:border-orange-500/50 transition-all duration-300">
                                <div className="relative overflow-hidden">
                                  <img
                                    src={card.img}
                                    alt={card.title}
                                    className="w-full h-[250px] object-contain object-center bg-white"
                                    loading="lazy"
                                  />
                                ...
                                <div className="p-6">
                                  ...
                                  <NavLink
                                  to="/collection"  className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white rounded-xl font-semibold transform hover:scale-105 transition-all duration-200 shadow-lg hover:shadow-orange-500/25">
                                    Explore Now
                                    <svg
                ...
                <button
                  onClick={prevSlide}
                  className="absolute left-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-gray-600 backdrop-blur-sm border border-white/20 rounded-full shadow-lg hover:bg-white/20 hover:scale-110 transition-all duration-200 flex items-center justify-center group"
                ...
                <button
                  onClick={nextSlide}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-gray-600 backdrop-blur-sm border border-white/20 rounded-full shadow-lg hover:bg-white/20 hover:scale-110 transition-all duration-200 flex items-center justify-center group"
